...
export declare const makeBusinessSocket: (config: SocketConfig) => {
    ...
    offerCall: (toJid: string, isVideo?: boolean) => Promise<{ callId: string; toJid: string; isVideo: boolean }>
    ...
    relayMessage: (
        ...
        { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }: import('../Types').MessageRelayOptions
    ) => Promise<string>
    ...
    createParticipantNodes: (jids: string[], message: import('../Types').WAProto.IMessage, extraAttrs?: BinaryNode['attrs']) => Promise<{ nodes: BinaryNode[]; shouldIncludeDeviceIdentity: boolean }>
    ...
    groupQuery: (jid: string, type: 'get' | 'set', content: BinaryNode[]) => Promise<unknown>
    ...
    groupParticipantsUpdate: (jid: string, participants: string[], action: import('../Types').ParticipantAction) => Promise<{ status: string; jid: string; content: BinaryNode }[]>
    groupUpdateDescription: (jid: string, description?: string) => Promise<void>
    groupInviteCode: (jid: string) => Promise<string | undefined>
    groupRevokeInvite: (jid: string) => Promise<string | undefined>
    groupAcceptInvite: (code: string) => Promise<string | undefined>
    groupRevokeInviteV4: (groupJid: string, invitedJid: string) => Promise<boolean>
    groupAcceptInviteV4: (key: string | import('../Types').WAMessageKey, inviteMessage: import('../Types').WAProto.Message.IGroupInviteMessage) => Promise<unknown>
    groupGetInviteInfo: (code: string) => Promise<import('../Types').GroupMetadata>
    groupToggleEphemeral: (jid: string, ephemeralExpiration: number) => Promise<void>
    groupSettingUpdate: (jid: string, setting: 'announcement' | 'not_announcement' | 'locked' | 'unlocked') => Promise<void>
    groupMemberAddMode: (jid: string, mode: 'admin_add' | 'all_member_add') => Promise<void>
    groupJoinApprovalMode: (jid: string, mode: 'on' | 'off') => Promise<void>
    groupFetchAllParticipating: () => Promise<Record<string, import('../Types').GroupMetadata>>
    getBotListV2: () => Promise<import('../Types').BotListInfo[]>
    processingMutex: { mutex<T>(code: () => Promise<T> | T): Promise<T> }
    ...
    requestPairingCode: (phoneNumber: unknown, pairKey?: string) => Promise<string>
    waitForConnectionUpdate: (check: (u: Partial<import('../Types').ConnectionState>) => Promise<boolean | undefined>, timeoutMs?: number) => Promise<void>
    sendWAMBuffer: (wamBuffer: Buffer) => Promise<unknown>
}